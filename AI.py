...
SCORE_FIVE, SCORE_FOUR, SCORE_SFOUR = 100000, 10000, 200
...
class ChessAI():
	...
	def findBestChess(self, board, turn):
		...
		time2 = time.time()
		DEBUG(DEBUG_WARN, 'time:%.2f step:%d position:(%d, %d), α=%d β=%d queue:%d score:%d' % ((time2-time1), self.number, x, y, self.alpha, self.belta, self.save_count, score))
		return (x, y)
	...
	def evaluatePoint(self, board, x, y, mine, opponent, count=None):
		...
		for i in range(4):
			if self.record[y][x][i] == 0 or ignore_record:
				self.analyseLine1(board, x, y, i, dir_offset[i], mine, opponent, count)
			else:
				self.save_count += 1
	...
	def analyseLine1(self, board, x, y, dir_index, dir, mine, opponent, count):
	...
	def analyseLine(self, board, x, y, dir_index, dir, mine, opponent):
		empty = MAP_ENTRY_TYPE.MAP_EMPTY.value
